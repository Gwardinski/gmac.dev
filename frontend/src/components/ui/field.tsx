...
const fieldVariants = cva('data-[invalid=true]:text-red-600 gap-2 group/field flex w-full', {
  variants: {
    orientation: {
...
function FieldLabel({ className, ...props }: React.ComponentProps<typeof Label>) {
  return (
    <Label
      ...
      className={cn(
        'group/field-label peer/field-label flex w-fit gap-2 leading-snug group-data-[disabled=true]/field:opacity-50 has-data-checked:border-blue-500 has-data-checked:bg-blue-50 has-[>[data-slot=field]]:rounded-lg has-[>[data-slot=field]]:border dark:has-data-checked:bg-blue-900/20 [&>*]:data-[slot=field]:p-2.5',
...
function FieldDescription({ className, ...props }: React.ComponentProps<'p'>) {
  return (
    <p
      ...
      className={cn(
        'text-left text-sm leading-normal font-normal text-gray-600 group-has-[[data-orientation=horizontal]]/field:text-balance dark:text-gray-400 [[data-variant=legend]+&]:-mt-1.5',
...
function FieldSeparator({
  ...
  return (
    <div data-slot="field-separator" data-content={!!children} className={cn('relative -my-2 h-5 text-sm group-data-[variant=outline]/field-group:-mb-2', className)} {...props}>
      ...
      {children && (
        <span className="relative mx-auto block w-fit bg-white px-2 text-gray-600 dark:bg-gray-950 dark:text-gray-400" data-slot="field-separator-content">
          {children}
        </span>
      )}
    </div>
...
function FieldError({
  ...
  return (
    <div role="alert" data-slot="field-error" className={cn('text-sm font-normal text-red-600', className)} {...props}>
      {content}
    </div>
  );
}
